...
// Shared with the former CreateUserModal — keep the form markup declarative
// and consistent with the app's design tokens.
...
/**
 * Full-page create/edit form for Student / Teacher / Institute-Admin users.
 * Replaces the former CreateUserModal — the 30+ field form had outgrown a modal.
 *
 * Context is passed in via route state from the launching page:
 *   navigate('/user-form', { state: {
 *     userRole, defaultInstituteId, defaultInstitute, editMode, existingUser, returnTo
 *   }})
 * On submit it returns to `returnTo` (which remounts and reloads its list).
 */
const UserForm = () => {
  ...
  // Without route state we have no context (e.g. page was refreshed or hit
  // directly) — bounce back to a safe place rather than render an ambiguous form.
  const hasContext = !!location.state && !!state.userRole;
  ...
  useEffect(() => {
    if (!hasContext) {
      const fallbackRole = (userProfile || user || {}).role;
      navigate(returnTo || getDashboardRoute(fallbackRole), { replace: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Load institutes (for the dropdown when no institute is pre-assigned).
  useEffect(() => {
    if (hasContext) loadInstitutes();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Load courses + batches for the student assignment dropdowns.
  useEffect(() => {
    if (hasContext && userRole === 'STUDENT') {
      loadCourses();
      loadBatches();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Populate the form from the existing user (edit) or defaults (create).
  useEffect(() => {
    if (editMode && existingUser) {
      setFormData({
        ...
        // Hydrate the independent course / batch sets from the student's DTOs.
  ...
  const loadInstitutes = async () => {
    ...
    try {
      ...
      setInstitutes(data && !error && Array.isArray(data) ? data : []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Carry role-specific fields through to the API.
      ...
      if (error) {
        ...
        // Success — go back to the launching list (it remounts and reloads).
        navigate(returnTo || getDashboardRoute(userRole), { replace: true });
      }
    } catch (err) {
  ...
  const pageTitle = `${editMode ? 'Edit' : 'Create New'} ${roleWord}`;
  ...
  return (
    <PageLayout title={pageTitle}>
      <div className="p-4 sm:p-6 pb-28 max-w-3xl mx-auto">
        ...
        <div className="mb-6 flex items-start gap-3">
          <button
            ...
            onClick={goBack}
            aria-label="Go back"
            className="mt-1 inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
          >
      ...
      {/* Sticky action bar */}
      <div className="sticky bottom-0 z-10 border-t border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
