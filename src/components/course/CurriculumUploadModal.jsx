...
const CurriculumUploadModal = ({ isOpen, onClose, onUploaded }) => {
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-lg border border-border shadow-lg w-full max-w-3xl max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-foreground">Bulk Upload Curriculum (CSV)</h2>
            <button onClick={handleClose} className="text-muted-foreground hover:text-foreground">
              <Icon name="X" size={24} />
            </button>
          </div>
          <p className="text-muted-foreground mt-2">
            Upload a single CSV to create subjects, chapters and topics at once. Existing entities are
            matched and reused by their code.
          </p>
        </div>
